...
obstacle_initialized_(false),
obstacle_added_(false),
is_goal_completed(false)
{
  ...
  StaticObjLinkIdPub = nh.advertise<obj_msgs::ObjList>("/StaticObj_link_id", 1, true);
  GoalTestPub = nh.advertise<visualization_msgs::Marker>("/Goal_testing_marker",1,true);

//
  DebugMarker1Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker1",1,true);
  DebugMarker2Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker2",1,true);
  DebugMarker3Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker3",1,true);
  DebugMarker4Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker4",1,true);
  DebugMarker5Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker5",1,true);
  DebugMarker6Pub = nh.advertise<visualization_msgs::Marker>("/debug_static_objMarker6",1,true);
  ...
  static ros::Subscriber avoidance_sub = nh.subscribe("/replan_request",1,&PLANNING::avoidance_callback,this);
  // static ros::Subscriber BaseLinkOffSet = nh.subscribe("/base_link_offset", &PLANNING::BaseLinkOffSetCallback, this);
  // vector_globalObj_link_str.push_back("155M0139RZ0204");

  ros::Rate rate(10);
  while(ros::ok())
  {
    ros::spinOnce();
    if (map_initialized_ && mission_initialized_ && obstacle_initialized_)
    {
      if (!obstacle_added_)
      {
        obstacleThread();
        obstacle_added_ = true;
      }
      missionThread();
      mission_initialized_ = false;
    }
...
void PLANNING::obstacleThread()
{
    ...
    for(int i=0; i<received_objlist.objlist.size(); i++)
    {
      tmp_point.position.x = (received_objlist.objlist[i].ul.x + received_objlist.objlist[i].ur.x) / 2.0;
      tmp_point.position.y = (received_objlist.objlist[i].ul.y + received_objlist.objlist[i].ur.y) / 2.0;
      tmp_points.push_back(tmp_point);
      tmp_point.position.x =(received_objlist.objlist[i].ul.x + received_objlist.objlist[i].bl.x) / 2.0;
      tmp_point.position.y =(received_objlist.objlist[i].ul.y + received_objlist.objlist[i].bl.y) / 2.0;
      tmp_points.push_back(tmp_point);
      tmp_point.position.x =(received_objlist.objlist[i].bl.x + received_objlist.objlist[i].br.x) / 2.0;
      tmp_point.position.y =(received_objlist.objlist[i].bl.y + received_objlist.objlist[i].br.y) / 2.0;
      tmp_points.push_back(tmp_point);
      tmp_point.position.x =(received_objlist.objlist[i].ur.x + received_objlist.objlist[i].br.x) / 2.0;
      tmp_point.position.y =(received_objlist.objlist[i].ur.y + received_objlist.objlist[i].br.y) / 2.0;
      tmp_points.push_back(tmp_point);

      tmp_point.position.x = (received_objlist.objlist[i].ul.x + received_objlist.objlist[i].ur.x + received_objlist.objlist[i].bl.x + received_objlist.objlist[i].br.x) / 4.0;
      tmp_point.position.y = (received_objlist.objlist[i].ul.y + received_objlist.objlist[i].ur.y + received_objlist.objlist[i].bl.y + received_objlist.objlist[i].br.y) / 4.0;
      center_obj.push_back(tmp_point);
    }
    std::cout << "Done 1st For loop" << std::endl;
    ...
    for(int i=0; i< center_obj.size(); i++)
    {
      for(int j=0; j<4; j++)
      {
        ...
        if(i==0)
        {
          ...
          // printf("dist1: %f\n",GetDistance(tmp_points[cnt].position.x, tmp_points[cnt].position.y, center_obj[i].position.x, center_obj[i].position.y));
        }
        else if(i==1)
        {
    ...
    std::cout << "Done 2nd For loop" << std::endl;
    ...
    // std::cout << "Obj1 Index1: " << obj1_maxIndex1 << "   Obj2 Index2: " << obj1_maxIndex2 << std::endl;
    // std::cout << "Obj2 Index1: " << obj2_maxIndex1 << "   Obj2 Index2: " << obj2_maxIndex2 << std::endl;
    ...
    dX2 = center_obj[1].position.x - tmp_points[obj2_maxIndex1+4].position.x;
    dY2 = center_obj[1].position.y - tmp_points[obj2_maxIndex1+4].position.y;
    yaw1 = atan2(dY1, dX1);
    yaw2 = atan2(dY2, dX2);
    ...
    tf2::convert(q2_rot, tmp_points[obj2_maxIndex1+4].orientation);
    tf2::convert(q2_rot, tmp_points[obj2_maxIndex2+4].orientation);
    //

    vector_globalObj_pose.clear();
    ...
    vector_globalObj_pose.push_back(tmp_points[obj2_maxIndex1+4]);
    vector_globalObj_pose.push_back(tmp_points[obj2_maxIndex2+4]);

    // for(int i=0; i<vector_globalObj_pose.size(); i++)
    // {
    //   std::cout << i << "th" << std::endl;
    //   std::cout << vector_globalObj_pose[i].position.x - OffsetMapX << std::endl;
    //   std::cout << vector_globalObj_pose[i].position.y - OffsetMapY << std::endl;
    // }

    visualization_msgs::Marker DebugMarker1;
    visualization_msgs::Marker DebugMarker2;
    visualization_msgs::Marker DebugMarker3;
    visualization_msgs::Marker DebugMarker4;
    visualization_msgs::Marker DebugMarker5;
    visualization_msgs::Marker DebugMarker6;

    DebugMarker1.header.frame_id = "map";
    DebugMarker1.header.stamp = ros::Time::now();
    DebugMarker1.action = visualization_msgs::Marker::ADD;
    DebugMarker1.pose.orientation.w = 1.0;
    DebugMarker1.id = 606;
    DebugMarker1.type = visualization_msgs::Marker::CUBE;
    DebugMarker1.scale.x = 2.0;
    DebugMarker1.scale.y = 2.0;
    DebugMarker1.scale.z = 2.0;
    DebugMarker1.color.g = 0.0;
    DebugMarker1.color.b = 0.5;
    DebugMarker1.color.r = 0.5;
    DebugMarker1.color.a = 1.0;

    DebugMarker2.header.frame_id = "map";
    DebugMarker2.header.stamp = ros::Time::now();
    DebugMarker2.action = visualization_msgs::Marker::ADD;
    DebugMarker2.pose.orientation.w = 1.0;
    DebugMarker2.id = 607;
    DebugMarker2.type = visualization_msgs::Marker::CUBE;
    DebugMarker2.scale.x = 2.0;
    DebugMarker2.scale.y = 2.0;
    DebugMarker2.scale.z = 2.0;
    DebugMarker2.color.g = 0.0;
    DebugMarker2.color.b = 0.5;
    DebugMarker2.color.r = 0.5;
    DebugMarker2.color.a = 1.0;

    DebugMarker3.header.frame_id = "map";
    DebugMarker3.header.stamp = ros::Time::now();
    DebugMarker3.action = visualization_msgs::Marker::ADD;
    DebugMarker3.pose.orientation.w = 1.0;
    DebugMarker3.id = 608;
    DebugMarker3.type = visualization_msgs::Marker::CUBE;
    DebugMarker3.scale.x = 2.0;
    DebugMarker3.scale.y = 2.0;
    DebugMarker3.scale.z = 2.0;
    DebugMarker3.color.g = 0.0;
    DebugMarker3.color.b = 0.5;
    DebugMarker3.color.r = 0.5;
    DebugMarker3.color.a = 1.0;

    DebugMarker4.header.frame_id = "map";
    DebugMarker4.header.stamp = ros::Time::now();
    DebugMarker4.action = visualization_msgs::Marker::ADD;
    DebugMarker4.pose.orientation.w = 1.0;
    DebugMarker4.id = 609;
    DebugMarker4.type = visualization_msgs::Marker::CUBE;
    DebugMarker4.scale.x = 2.0;
    DebugMarker4.scale.y = 2.0;
    DebugMarker4.scale.z = 2.0;
    DebugMarker4.color.g = 0.0;
    DebugMarker4.color.b = 0.5;
    DebugMarker4.color.r = 0.5;
    DebugMarker4.color.a = 1.0;

    DebugMarker5.header.frame_id = "map";
    DebugMarker5.header.stamp = ros::Time::now();
    DebugMarker5.action = visualization_msgs::Marker::ADD;
    DebugMarker5.pose.orientation.w = 1.0;
    DebugMarker5.id = 610;
    DebugMarker5.type = visualization_msgs::Marker::CUBE;
    DebugMarker5.scale.x = 2.0;
    DebugMarker5.scale.y = 2.0;
    DebugMarker5.scale.z = 2.0;
    DebugMarker5.color.g = 0.0;
    DebugMarker5.color.b = 0.5;
    DebugMarker5.color.r = 0.5;
    DebugMarker5.color.a = 1.0;

    DebugMarker6.header.frame_id = "map";
    DebugMarker6.header.stamp = ros::Time::now();
    DebugMarker6.action = visualization_msgs::Marker::ADD;
    DebugMarker6.pose.orientation.w = 1.0;
    DebugMarker6.id = 611;
    DebugMarker6.type = visualization_msgs::Marker::CUBE;
    DebugMarker6.scale.x = 2.0;
    DebugMarker6.scale.y = 2.0;
    DebugMarker6.scale.z = 2.0;
    DebugMarker6.color.g = 0.0;
    DebugMarker6.color.b = 0.5;
    DebugMarker6.color.r = 0.5;
    DebugMarker6.color.a = 1.0;

    DebugMarker1.pose.position.x = vector_globalObj_pose[0].position.x - map_x;
    DebugMarker1.pose.position.y = vector_globalObj_pose[0].position.y - map_y;
    DebugMarker2.pose.position.x = vector_globalObj_pose[1].position.x - map_x;
    DebugMarker2.pose.position.y = vector_globalObj_pose[1].position.y - map_y;
    DebugMarker3.pose.position.x = vector_globalObj_pose[2].position.x - map_x;
    DebugMarker3.pose.position.y = vector_globalObj_pose[2].position.y - map_y;
    DebugMarker4.pose.position.x = vector_globalObj_pose[3].position.x - map_x;
    DebugMarker4.pose.position.y = vector_globalObj_pose[3].position.y - map_y;
    DebugMarker5.pose.position.x = vector_globalObj_pose[4].position.x - map_x;
    DebugMarker5.pose.position.y = vector_globalObj_pose[4].position.y - map_y;
    DebugMarker6.pose.position.x = vector_globalObj_pose[5].position.x - map_x;
    DebugMarker6.pose.position.y = vector_globalObj_pose[5].position.y - map_y;

    DebugMarker1Pub.publish(DebugMarker1);
    DebugMarker2Pub.publish(DebugMarker2);
    DebugMarker3Pub.publish(DebugMarker3);
    DebugMarker4Pub.publish(DebugMarker4);
    DebugMarker5Pub.publish(DebugMarker5);
    DebugMarker6Pub.publish(DebugMarker6);

    std::cout << "Done publish marker" << std::endl;


   pair<string, geometry_msgs::Point> obj1_for_pub, obj2_for_pub;
   center_obj[0].position.x = center_obj[0].position.x - map_x;
   center_obj[0].position.y = center_obj[0].position.y - map_y;
   center_obj[1].position.x = center_obj[1].position.x - map_x;
   center_obj[1].position.y = center_obj[1].position.y - map_y;
   std::cout << "obj1 " << std::endl;
   std::cout << center_obj[0] << std::endl;
   obj1_for_pub = FindClosestLink(center_obj[0]);
   obj2_for_pub = FindClosestLink(center_obj[1]);

   std::cout << "Center obj1 x: " << center_obj[0].position.x << "  Center obj1 y: " << center_obj[0].position.y << std::endl;
   std::cout << "Center obj2 x: " << center_obj[1].position.x << "  Center obj2 y: " << center_obj[1].position.y << std::endl;
   std::setprecision(10);
   std::cout << "x: " << map_x << "   y: " << map_y << std::endl;
   std::cout << "Found Link1: " << obj1_for_pub.first << std::endl;
   std::cout << "Found Link2: " << obj2_for_pub.first << std::endl;

   StaticObj_list.objlist.clear();
   StaticObj.link_id = obj1_for_pub.first;
   StaticObj.ul = received_objlist.objlist[0].ul;
   StaticObj.ur = received_objlist.objlist[0].ur;
   StaticObj.bl = received_objlist.objlist[0].bl;
   StaticObj.br = received_objlist.objlist[0].br;
   StaticObj_list.objlist.push_back(StaticObj);
   StaticObj.link_id = obj2_for_pub.first;
   StaticObj.ul = received_objlist.objlist[1].ul;
   StaticObj.ur = received_objlist.objlist[1].ur;
   StaticObj.bl = received_objlist.objlist[1].bl;
   StaticObj.br = received_objlist.objlist[1].br;
   StaticObj_list.objlist.push_back(StaticObj);

   StaticObjLinkIdPub.publish(StaticObj_list);

    // FIND Closest LINK
   vector_globalObj_link.clear();
    for(int i=0; i<vector_globalObj_pose.size(); i++)
    {
      vector_globalObj_pose[i].position.x = vector_globalObj_pose[i].position.x - map_x;
      vector_globalObj_pose[i].position.y = vector_globalObj_pose[i].position.y - map_y;
    ...
    vector_globalObj_link_str.clear();
    ...
    std::cout << "vector_globalObj_link_str size: " << vector_globalObj_link_str.size() << std::endl;
    std::cout << "static Obj Callback Done " <<std::endl;
}

void PLANNING::missionThread()
{
  ...
  vector_calculated_missions_final.clear();
  vector_calculated_missions.clear();
  vector_calculated_missions2start.clear();
  vector_closest_start_vehicle.clear();

  // VehiclePosition = mission_msg.curr_pos.pose.pose;

  // DONE

  pair<int, int> tmp;
  // 승차 위치 -> 하차 위치 Planning (항상 고정) mission_flag변수를 이용해서 한번만 실행
  // 제외 되어야 할 미션 id 리스트 생성
  tf2::Quaternion q;
  q.setRPY(1, 0, 0);
  q.normalize();
  if(mission_flag == false)
  {
    ...
    std::cout << "Vector Size: " << vector_mission.size() << std::endl;
    for(int i=0; i<vector_mission.size(); i++)
    {
      // Set Start & Goal
      geometry_msgs::Pose goal,start;
      goal.position = vector_mission[i].end_pos.pose.position;
      goal.orientation = vector_mission[i].end_pos.pose.orientation;
      goal.position.x = goal.position.x - map_x;
      goal.position.y = goal.position.y - map_y;
      start.position = vector_mission[i].start_pos.pose.position;
      start.orientation = vector_mission[i].end_pos.pose.orientation;
      start.position.x = start.position.x - map_x;
      start.position.y = start.position.y - map_y;
      ...
      std::cout << "Start Generate global path() " << std::endl;
      GenerateGlobalPath(); // trajectory.waypoints.clear() 실행
      std::cout << "end generateglobalpath() " << std::endl;
      if(closed_list.size() !=0)
      {
        GenerateGoalPoint();
        std::cout << "Done GenerateGoalPoint() " << std::endl;
      }
      if(trajectory.waypoints.size() !=0)
      {
        correctWaypoints(trajectory); // global path trajectory 실행 완료
        std::cout << "Done correctWaypoints() " << std::endl;
      }
      tmp.first = vector_mission[i].mission_id;
  ...
  std::cout << "TESTING DEBUG1 " << std::endl;
  ...
  for(int i=0; i<vector_mission.size(); i++)
  {
    ...
    goal = vector_mission[i].start_pos.pose;
    start.position.x = start.position.x - map_x;
    start.position.y = start.position.y - map_y;
    goal.position.x = goal.position.x - map_x;
    goal.position.y = goal.position.y - map_y;
  ...
  // std::cout For DEBUGGING COST
  for(int i=0; i< vector_mission.size(); i++)
  {
    pair<int, int> tmp;
    tmp.second = vector_mission[i].mission_id;
    if(vector_calculated_missions2start[i].second != 0 && vector_calculated_missions[i].second !=0)
    {
      tmp.first = vector_calculated_missions2start[i].second + vector_calculated_missions[i].second;
      vector_calculated_missions_final.push_back(tmp);
    }
    // std::cout << "2start" << "  ID: " << vector_calculated_missions2start[i].first << "   SIZE:  " << vector_calculated_missions2start[i].second << std::endl;
    // std::cout << "start2goal" << "  ID: " << vector_calculated_missions[i].first << "    SIZE: " << vector_calculated_missions[i].second << std::endl;
    // std::cout << "total" << "ID: " << i << "   SIZE: " << tmp.first << std::endl;

  }

  sort(vector_calculated_missions_final.begin(), vector_calculated_missions_final.end());
  ...
  {
    // std::cout << "ID: " << vector_calculated_missions_final[i].second << std::endl;
    // std::cout << "SIZE: " << vector_calculated_missions_final[i].first << std::endl;

    global2mission_msg.id = vector_calculated_missions_final[i].second;
    std::cout << "id: " << global2mission_msg.id << std::endl;
    global2mission_msg.num_point = vector_calculated_missions_final[i].first;
    global2mission_msg_list.lists.push_back(global2mission_msg);

  }

  //  거리에 따른 mission 벡터 생성 후 publish

  Global2MissionPub.publish(global2mission_msg_list);
}

void PLANNING::avoidance_callback(const path_msgs::Avoidance msg)
{

  // 1. Get Vehicle Pose
  // 2. Get Obstacle Link String
  // 3. Goal Pose
  // 4. Previous left Path Size
  if(is_goal_completed)
  {
    std::cout << "Start Avoidance" << std::endl;
    // Functions
    VehiclePosition = msg.vehicle_pose;
    // save_goal_pose  RvizGoal Result in handler (V2X)
    ClosestStartLink = FindClosestLink(VehiclePosition);
    ClosestEndLink = FindClosestLink(save_goal_pose);

    std::cout << "Goal: " << save_goal_pose << std::endl;
    ...
    // Check State and Set Start Node and Goal Node
    if(start_goal_flag == 1)
    {
      start_node_id = links.at(ClosestStartLink.first).to_node;
      goal_node_id = links.at(ClosestEndLink.first).to_node;
    ...
    veh_obj_str = FindClosestLink(msg.obj_pose).first;
    GenerateGlobalPath(); // Update Openlist 포함하고있음
    if(closed_list.size() !=0)
    {
      GenerateGoalPoint();
    }
    ...
    if(trajectory.waypoints.size() < msg.left_traj + 100 && trajectory.waypoints.size() !=0)
    {
    //PUBLISH TRAJECTORY
    //Drawing Path
    trajectory.goal_link = links.at(ClosestEndLink.first);
    TrajectoryPub.publish(trajectory);
    DrawingPath();
    std::cout << "Operate Avoidance " <<std::endl;
    }
    else
    {
...
{
  // std::cout << "Start Map info callback" << std::endl;
  ...
  std::cout << links_msg.size() << std::endl;
  std::cout << "lanes" << std::endl;
  std::cout << lanes_msg.size() << std::endl;
  std::cout << "nodes" << std::endl;
  std::cout << nodes_msg.size() << std::endl;
  std::cout << "map_initialized_: " << map_initialized_ << std::endl;
}
...
void PLANNING::currentOdomCallback(const nav_msgs::Odometry gps_odom)
{
  if(map_initialized_)
  {
    ...
    // Calc Dist
    float calc_dist = GetDistance(ClosestEndLink.second.x, ClosestEndLink.second.y, VehiclePosition.position.x, VehiclePosition.position.y );
    // if(calc_dist < 10 )
      // std::cout << "Left Distance : " << calc_dist << std::endl;
  }
...
void PLANNING::GenerateGlobalPath(){
  ...
  while( !is_complete_path ){
    ...
    count_number++;
  	if( current_node.NLIDS[0] != "" ){
        ...
        if(current_link.RLID[0] == ClosestEndLink.first)
        {
            bezier_threshold = 5;
        }
        else
        {
            bezier_threshold = 35;
  			...
  			if( current_link.RLID[0] != "" && links.at(current_link.RLID[0]).length > bezier_threshold  ){ //35
        ...
        if(current_link.LLID[0] == ClosestEndLink.first)
        {
            bezier_threshold = 5;
        }
        else
        {
            bezier_threshold = 35;
  			...
  			if( current_link.LLID[0] != "" && links.at(current_link.LLID[0]).length > bezier_threshold ){ //35
  ...
  //
    ...
    // std::cout << "num: " << count_number << std::endl;
    if(count_number > 200)
    {
      closed_list.clear();
      break;
...
void PLANNING::UpdateOpenList(Cost temp_cost){
    ...
    if( is_new_open_list ){
        ...
        // std::cout << "global obj: " << vector_globalObj_link_str[0] << std::endl;
        if(open_list.size() != 0 )
        {
          for(int l=0; l < vector_globalObj_link_str.size(); l++)
          {
            if(open_list.back().link == vector_globalObj_link_str[l] )
            {
              open_list.pop_back();
            }
          }
        }
        if(veh_obj_str == open_list.back().link && veh_obj_str.size() !=0)
        {
          open_list.pop_back();
        }
    }
        ...
        // if (i < trajectory_node.size()-1 && i > 0 ){
            if(is_lane_changed) {
                ...
                for(int j=0; j<waypoint_link.geometry.size(); j++){
                    ...
                    wp.point.x = waypoint_link.geometry[j].x - map_x;
                    wp.point.y = waypoint_link.geometry[j].y - map_y;
...
void PLANNING::DrawingPath(){
    ...
    PathArray.id =335;
    PathArray.type = visualization_msgs::Marker::POINTS;
    PathArray.scale.x = 0.3;
    PathArray.scale.y = 0.3;
    PathArray.color.r = 34.0/255;
    PathArray.color.g = 139.0/255;
    PathArray.color.b = 34.0/255;
    PathArray.color.a = 1.0;

    PathArray.points.clear();
    for(int i=0; i<trajectory.waypoints.size(); i++)
    {
        trajectory.waypoints[i].point.z -= 0.2;
...
GoalTestPub.publish(goal_test_marker);
  ...
  if (map_initialized_)
  {
    ...
    if(trajectory.waypoints.size() !=0)
    {
      ...
      printf("Goal link sent: %s\n", ClosestEndLink.first.c_str());
      trajectory.goal_link = links.at(ClosestEndLink.first);
      TrajectoryPub.publish(trajectory);
...
pair<string, geometry_msgs::Point> PLANNING::FindClosestLink(geometry_msgs::Pose CurPose){
    ...
    float min_dist = 100;
    geometry_msgs::Point link_point_first, link_point_last;
    // std::cout << "RUN FIND closest link " << std::endl;

    for (auto it=links.begin(); it!=links.end(); it++){
        int i_first = 0;
        int i_last = it->second.geometry.size()-1;
        link_point_first = it->second.geometry[i_first];
            link_point_first.x -= map_x;
            link_point_first.y -= map_y;
        link_point_last = it->second.geometry[i_last];
            link_point_last.x -= map_x;
            link_point_last.y -= map_y;
        float dist1 = GetDistance(CurPose.position.x, CurPose.position.y, link_point_first.x, link_point_first.y);
        float dist2 = GetDistance(CurPose.position.x, CurPose.position.y, link_point_last.x, link_point_last.y);
        ...
        //     continue;
        //     // this link's end nodes are both too far from current position to be considered
        // }


        for (auto it1=it->second.geometry.begin(); it1!=it->second.geometry.end(); it1++){
            // cout << "===Enter ENTER to continue===";
            // if(cin.get() == '\n'){cout<<endl;}

            geometry_msgs::Point tmp_Point;
                tmp_Point.x = it1->x - map_x;
                tmp_Point.y = it1->y - map_y;
                tmp_Point.z = it1->z;
            float tmp_dist = GetDistance(CurPose.position.x, CurPose.position.y, tmp_Point.x, tmp_Point.y);
            // std::cout << "tmp_dist: " << tmp_dist << std::endl;

            // geometry_msgs::PointStamped aaaaa_point;
            // aaaaa_point.header.frame_id = "map";
            // aaaaa_point.point = tmp_Point;
            // aaaaa.publish(aaaaa_point);

            if (tmp_dist < min_dist){
                selected_link.first = it->second.id;
                selected_link.second = tmp_Point;
                min_dist = tmp_dist;
                // cout << "new min dist of " << min_dist << endl;
            }
        }
    }
...
// pair<string, geometry_msgs::Point> PLANNING::FindClosestLink(geometry_msgs::Pose CurPose)
// {
//   // Set Pair<double, int> -> distance , id btw given pose and node
//   // Find Closeset 5 Nodes
//   // Get Next and Previous Link from 5 Nodes
//   // Find distance to the Closest Point at each link
//   // Compare distances to find the closeset link
// }
...
void PLANNING::GenerateGoalPoint(){
    ...
    else
    {
        ...
        if(is_within)
        {
            if(trajectory.waypoints[0].point.x != (goal_link.geometry[0].x - map_x))
            {
                for(int i=0; i<checkidx+1; i++)
                {
                    ...
                    wp.point.x = goal_link.geometry[i].x - map_x;
                    wp.point.y = goal_link.geometry[i].y - map_y;
        ...
        else // Next Link 찾아야함 ROADTYPE 8,9는 회전 Link, 따라서 회전 링크의 경우 제외
        {
            ...
            if(nextlink != "")
            {
                ...
                if(trajectory.waypoints[0].point.x != (goal_link.geometry[0].x - map_x))
                {
                    ...
                    {
                        wp.CLID = nextlink;
                        wp.point.x = links.at(nextlink).geometry[i].x - map_x;
                        wp.point.y = links.at(nextlink).geometry[i].y - map_y;
                        trajectory.waypoints.push_back(wp);
                    }
                }
                // // 같은 링크일떄
...
void PLANNING::GenerateStartPoint(bool is_lane_changed){
    ...
    if(is_lane_changed){
        ...
        for(int i=0; i<to_link.geometry.size(); i++){
            ...
            wp.point.x = to_link.geometry[i].x - map_x;
            wp.point.y = to_link.geometry[i].y - map_y;
        ...
        if(ClosestStartLink.first == ClosestEndLink.first){
            if(start_goal_flag != 1)
            {
              ...
              {
                  calcdist = GetDistance(tmp.geometry[i].x, tmp.geometry[i].y, ClosestEndLink.second.x + map_x, ClosestEndLink.second.y + map_y);
                  calcdist_start = GetDistance(tmp.geometry[i].x, tmp.geometry[i].y, VehiclePosition.position.x + map_x, VehiclePosition.position.y + map_y);
                  if(calcdist < mindist)
                  {
                      closestidx = i;
                      mindist = calcdist;
              ...
              for(int i=0; i<closestidx; i++)
              {
                ...
                wp.point.x = tmp.geometry[i].x - map_x;
                wp.point.y = tmp.geometry[i].y - map_y;
                wp.speed = tmp.speed;
                trajectory.waypoints.push_back(wp);
              }
            }
            else
            {
              ...
              {
                calcdist = GetDistance(temp_link.geometry[i].x, temp_link.geometry[i].y, first_closest_link.second.x + map_x, first_closest_link.second.y + map_y);
                if(calcdist < mindist)
                {
                  closestidx = i;
                  mindist = calcdist;
              ...
              for(int i=closestidx; i<links.at(first_closest_link.first).geometry.size(); i++)
              {
                ...
                wp.RLID = start_link.RLID[0];
                wp.LLID = start_link.LLID[0];
                wp.point.x = temp_link.geometry[i].x - map_x;
                wp.point.y = temp_link.geometry[i].y - map_y;
...
Trajectory PLANNING::GoalLinkChangeLaneTrajectory(Link from_link, Link to_link){
    ...
    // ROS_INFO_STREAM(P1.x-map_x << ", " << P1.y-map_y);
    // ROS_INFO_STREAM(P2.x-map_x << ", " << P2.y-map_y);
    // ROS_INFO_STREAM(P3.x-map_x << ", " << P3.y-map_y);
    // ROS_INFO_STREAM(P4.x-map_x << ", " << P4.y-map_y);
    ...
    wp.point.x = ClosestEndLink.second.x + map_x;
    wp.point.y =  ClosestEndLink.second.y + map_y;
    wp.CLID = to_link.id;
    wp.RLID = to_link.RLID[0];
    wp.LLID = to_link.LLID[0];
    wp.speed = to_link.speed;
    ...
    */
    ...
    geometry_msgs::Point tmpforidx;
    tmpforidx.x = curved_trajectory.waypoints.back().point.x - map_x;
    tmpforidx.y = curved_trajectory.waypoints.back().point.y - map_y;
    int closest_idx;
    closest_idx = SearchClosestIndex(tmpforidx, links.at(ClosestEndLink.first));
...
int PLANNING::SearchClosestIndex(geometry_msgs::Point point, Link link){
    ...
    for(int i=0; i<link.geometry.size()-1; i++){
        dist = GetDistance(point.x, point.y, link.geometry[i].x - map_x, link.geometry[i].y - map_y);
