...
terminate_thread_(false)
{
  ...
  private_nh_.param<double>("vel_offset", vel_offset_, 8.0);
  private_nh_.param<double>("deadend_thres", deadend_thres_, 1.5);
  ...
  private_nh_.param<double>("wait_time_thres", node_wait_time_thres_, 3.0);
  ...
  atypical_obj_sub_ = nh_.subscribe("/StaticObj_link_id", 1, &DynamicPlanning::atypicalObjCallback, this);
  estop_status_sub_ = nh_.subscribe("emergency_stop_status", 10, &DynamicPlanning::estopStatusCallback, this);
  ...
  rviz_status_pub_ = nh_.advertise<visualization_msgs::Marker>("/rviz/status", 1);
  road_state_pub_ = nh_.advertise<path_msgs::State>("/road_state", 1);
  estop_pub_ = nh_.advertise<std_msgs::UInt8>("/estop", 1);
  avoid_state_pub_ = nh_.advertise<path_msgs::State>("/avoidance_state", 1);
  replan_pub_ = nh_.advertise<path_msgs::Avoidance>("/replan_request", 1);
  reached_pub_ = nh_.advertise<std_msgs::UInt8>("/reached", 1);
...
void DynamicPlanning::estopStatusCallback(const std_msgs::UInt8 msg)
{
  estop_status_ = msg.data;
}
...
void DynamicPlanning::transformAtypicalObject(obj_msgs::Obj &obj)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    double dist =  getDistance(from_node.geometry.x, from_node.geometry.y, vertices[i].x, vertices[i].y);
    from_distances[i] = dist;
    // printf("%d: dist: %f\n", i, dist);
  }
  sort(from_indices.begin(), from_indices.end(), [&](int i, int j) {return from_distances[i] < from_distances[j];});

  // Find the previous and next index of the closest point to the from node
  int n_index = from_indices[0] + 1;
  int p_index = from_indices[0] - 1;
  int pp_index = from_indices[0] - 2;
  // printf("n_index: %d, p_index: %d, pp_index: %d\n", n_index, p_index, pp_index);
  if (n_index > 3)
  {
    n_index = 0;
  }
  ...
  if (p_index == from_indices[1])
  {
    obj.bl = vertices[p_index];
    obj.br = vertices[from_indices[0]];
    obj.ul = vertices[pp_index];
    obj.ur = vertices[n_index];
  }
  else if (n_index == from_indices[1])
  {
    obj.bl = vertices[from_indices[0]];
    obj.br = vertices[n_index];
    obj.ul = vertices[p_index];
    obj.ur = vertices[pp_index];
  }
}
...
void DynamicPlanning::vehCallback(const obj_msgs::ObjList msg)
{
  ...
  if (locate_success_ && trajectory_initialized_)
  {
    ...
    for(int i=0; i < msg.objlist.size(); i++)
    {
      ...
      // if (msg.objlist[i].type != OType::OPPOSITE)
      {
        ...
        if (gobj.index >= 0 && gobj.link_id != "")
        {
          if (abs(ego_.lat_offset) <= HALF_LANE && !is_bezier_ahead_)
          {
            double arc_diff = std::get<0>(locateObjectBackFromEgo(gobj));
        ...
        else
        {
          ...
          // if (gobj.index >= 0 && gobj.link_id != "")
          // {
          //   obj_msgs::Obj safety_front = gobj;
          //   obj_msgs::Obj safety_back = gobj;
          ...
          //     safety_front = getEllipse(safety_front, center, 1.0, true);
          //   }
          //   // move fake's center to x m and size up
          //   else
          //   {
          //     gobj = getEllipse(gobj, gobj.pose.position, 1.0, true);
          ...
          //       safety_front = getEllipse(safety_front, center, 1.0, true);
          //     }
          //     // center.x = (gobj.bl.x + gobj.br.x) / 2.0;
          //     // center.y = (gobj.bl.y + gobj.br.y) / 2.0;
          //     // safety_back = getEllipse(safety_back, center, 1.0, true);
          //   }
          //   visualizeEllipse(gobj);
          //   visualizeEllipse(safety_front);
          //   // visualizeEllipse(safety_back);
          //   veh_list_.push_back(gobj);
          //   veh_list_.push_back(safety_front);
          //   // veh_list_.push_back(safety_back);
          // }
      }
    }
    ...
    for(int i = 0; i < atypical_obj_list_.size(); i++)
    {
      ...
      if (obj.link_id != "")
      {
        // printf("Before -- ulx: %f, uly: %f, urx: %f, ury: %f, blx: %f, bly %f, brx: %f, bry: %f \n", obj.ul.x, obj.ul.y, obj.ur.x, obj.ur.y, obj.bl.x, obj.bl.y, obj.br.x, obj.br.y);
        // if located in the global
        transformAtypicalObject(obj);
        // printf("After -- ulx: %f, uly: %f, urx: %f, ury: %f, blx: %f, bly %f, brx: %f, bry: %f \n", obj.ul.x, obj.ul.y, obj.ur.x, obj.ur.y, obj.bl.x, obj.bl.y, obj.br.x, obj.br.y);
        obj = getEllipse(obj, obj.pose.position, 1.0, false);
...
void DynamicPlanning::trajectoryCallback(const path_msgs::Trajectory traj_msg)
{
  ...
  goal_link_ = traj_msg.goal_link;
  printf("Goal link id: %s\n", goal_link_.id.c_str());
  // if (replan_)
  // {
  //   replan_ = false;
  // }
...
void DynamicPlanning::generateStopNodes()
{
  ...
  for(int i = 0; i < all_stop_nodes_.size(); i++)
  {
    ...
    if (node.index > ego_.index && node.link_id != "" && node.lat_offset <= HALF_LANE)
    {
      if (node.index < min_index)
      {
        closest_node = node;
        closest_node.is_wait = true;
  ...
  {
    // 현재 차량이 Stop node에 가까이 있음
    if (getDistance(closest_node.pose.position.x, closest_node.pose.position.y, ego_.pose.position.x, ego_.pose.position.y) <= 20.0)
    {
      // 차량이 정차해있음
      if (ego_speed_ == 0.0)
      {
        ...
        if (node_timer_ == 0.0)
        {
          printf("%d: First\n");
          is_node_ = true;
          node_timer_ += ros::Time::now().toSec() - curr_time;
        }
        // 두 번째 이상 발견 시
        else
        {
          ...
          if (prev_node_.id == closest_node.id)
          {
            printf("%d: Same node\n");
            node_timer_ += curr_time - prev_time_;
          }
        ...
        prev_node_ = closest_node;

        // ROS_WARN("Waiting for node: %f s", node_timer_);
        printf("Waiting for node: %f s\n", node_timer_);

        // 차량이 설정한 값보다 오래 멈춰 있었으면
        if (node_timer_ > node_wait_time_thres_)
        {
          is_node_ = false;
        }
        // 아직 좀 더 오래 멈춰있어야 한다면
        ...
        {
          stop_nodes_.push_back(closest_node);
        }
      }
      // 가까이 있지만 정차하지 않았음
      else
  ...
  else if (getDistance(closest_node.pose.position.x, closest_node.pose.position.y, ego_.pose.position.x, ego_.pose.position.y) > 20.0)
  {
    is_node_ = true;
    node_timer_ = 0.0;
...
void DynamicPlanning::locateObj(obj_msgs::Obj &obj)
{
  if (ego_.index >= 0)
  {
    ...
    {
      obj.index = searchTrajIndex(obj, global_trajectory_, HALF_LANE);
      if (obj.index >= 0)
      {
...
void DynamicPlanning::checkAvoidance()
{
  ...
  for(int i = 0; i < veh_list_.size(); i++)
  {
    ...
    {
      double raw_tilt_distance = getDistance(obj.pose.position.x, obj.pose.position.y, obj_deadend_.pose.position.x, obj_deadend_.pose.position.y);
      if (raw_tilt_distance < 0.7)
      {
        printf("CA: deadend is wait\n");
        obj.is_wait = true;
      }
    }
    ...
    if (abs(obj.lat_offset) <= HALF_LANE)
    {
      ...
      if (is_veh)
      {
        ...
        double arc_diff = std::get<0>(locateObjectBackFromEgo(obj));
        printf("Obj %d: arc_diff\n", obj.id, obj.index, arc_diff);
        ...
        // else if (links_.at(obj.link_id).road_type == RType::INTERSECTION)
        // {
        ...
        if (laneStatus(clink) == LControl::KEEP && !clink.road_type == RType::INTERSECTION)
        {
          // ROS_WARN("No adjacent link (obj) --> WAIT");
          printf("No adjacent link (obj) --> WAIT\n");
          obj.is_wait = true;
        }
        // Ego at the current link KEEP
        if (laneStatus(links_.at(ego_.link_id)) == LControl::KEEP)
        {
          // ROS_WARN("No adjacent link (me) --> WAIT");
          printf("No adjacent link (ego)--> WAIT\n");
          obj.is_wait = true;
        }
  ...
  // if (veh_list_.size() >= 2)
  // {
  //   for(int i = 0; i < veh_list_.size()-1; i++)
  //   {
  //     if (veh_list_[i].type == OType::STATIC)
  //     {
  //       if (veh_list_[i].id == veh_list_[i+1].id )
  //       {
  //         bool is_wait = veh_list_[i].is_wait || veh_list_[i+1].is_wait;
  //         veh_list_[i].is_wait = is_wait;
  //         veh_list_[i+1].is_wait = is_wait;
  //         i++;
  //       }
  //     }
  //   }
...
void DynamicPlanning::visualizeStatus()
{
  visualization_msgs::Marker text_marker;
  std::string text = "Acc: ";
  char buffer[32];
  memset(buffer, 0, sizeof(buffer));
  sprintf(buffer, "%1f", acceleration_);
  ...
  if (status_ == DynamicPlanning::STATE::RELAYING_I)
  {
    text += "RELAYING I";
  ...
  else if (status_ == DynamicPlanning::STATE::RELAYING_II)
  {
    text += "RLEYAING II";
    text_marker.color.r = 1.0;
  ...
  else if (status_ == DynamicPlanning::STATE::RELAYING_III)
  {
    text_marker.color.r = 1.0;
  ...
  else if (status_ == DynamicPlanning::STATE::CRUISING)
  {
  ...
  else if (status_ == DynamicPlanning::STATE::BLOCKED)
  {
    text_marker.color.r = 1.0;
    text_marker.color.g = 0.0;
    text_marker.color.b = 0.0;
  ...
  else if (status_ == DynamicPlanning::STATE::HARD_BRAKE)
  {
    text_marker.color.r = 1.0;
    text_marker.color.g = 0.0;
    text_marker.color.b = 0.0;
  ...
  else if (status_ == DynamicPlanning::STATE::STOP_GO)
  {
...
void DynamicPlanning::startThread()
{
  ...
  if (trajectory_initialized_ && pose_initialized_)
  {
    ...
    {
      locate_success_ = true;
      is_bezier_ahead_ = isBezierAhead(50);
      ...
      if (state_ == DynamicPlanning::STATE::CRUISING)
      {
        ...
        status_ = DynamicPlanning::STATE::CRUISING;
      ...
      else if (state_ == DynamicPlanning::STATE::STOP_GO)
      {
        printf("=======Stop and go ========\n");
        // ROS_WARN("=======Stop and go=========");
      }
      // Search object ahead
      searchObjAhead();
      updateObjAhead();
      switchState();
      // /* Estop or Stop And Go*/
      // executeEStop(headway_obj_);
      // if (estop_status_ == (uint8_t)1)
      // {
      //   return;
      // }
    }
...
void DynamicPlanning::minAccel()
{
  printf("Minimum velocity\n");
  // status_ = DynamicPlanning::STATE::STOP_GO;
  std_msgs::Float64 msg;
  msg.data = 3.0;
...
void DynamicPlanning::setObsCost()
{
  ...
  // Get adjacent lanes
  std::tuple<std::vector<path_msgs::Lane>, std::vector<path_msgs::Lane>> adj_lanes = getAdjacentLanes(ego_.link_id); // 교차로 터짐
  ...
  for(int i = 0; i < candidates_.size(); i++)
  {
    ...
    for(int k = -path_size_; k <= path_size_; k++)
    {
      ...
      {
        g = 1.0 / sqrt(2*PI*sigma_*3) * exp(-pow(k,2) / (2*pow(sigma_*3, 2)));
      }
...
std::tuple<double, double> DynamicPlanning::locateObjectBackFromEgo(obj_msgs::Obj obj)
{
  ...
  if (is_veh)
  {
    ...
    {
      return locateObjectFromEgo(obj);
    }
  }
  else
  {
...
double DynamicPlanning::getSafetyDistance()
{
  ...
  //   path_msgs::State msg;
  //   msg.state = 1;
  //   avoid_state_pub_.publish(msg);
  //   if (!first_sight_)
  //   {
  //     // avoidance_distance_ = std::get<0>(locateObjectBackFromEgo(headway_obj_));
  //     // avoidance_distance_ = std::get<0>(locateObjectFromEgo(headway_obj_));
  //     avoidance_distance_ = 8.0;
  //     first_sight_ = true;
  //   }
  //   safety_distance = avoidance_distance_;
  //   // if (avoidance_distance_ > min_dist_)
  //   // {
...
void DynamicPlanning::findOptimalPath()
{
  ...
  for(int i=0; i<candidates_.size();i++)
  {
    ...
    if (cost_w < min_cost_w && R_all_[i] < 0.5)
    {
      min_index_w = i;
      min_cost_w = cost_w;
    }
    if (cost_wo < min_cost_wo && R_lanes_[i] < 0.5)
    {
      min_index_wo = i;
      min_cost_wo = cost_wo;
    }
    printf("%d cost: %3f, %3f, %3f %f --> %3f, %3f, %3f\n",
    i,
    wc_*(a_ * candidates_[i].smc + b_*candidates_[i].coc),
    ws_*candidates_[i].oc,
    wl_*candidates_[i].gc,
    ws_*candidates_[i].lc, cost_wo, cost_w, R_all_[i]);
  }
...
void DynamicPlanning::switchRelaying()
{
  ...
  else if (obj.type == OType::STATIC)
  {
    ...
    {
      printf("Do not avoid\n");
      // ROS_WARN("Do not avoid\n");
      is_brake_ = true;
      is_avoid_ = false;
      static_timer_ = 0.0;
      is_wait_status_ = true;
    }
    // 10 seconds pass then avoid
    else
    {
      is_wait_status_ = false;
      ...
      // {
        ...
        else
        {
          ...
          // is_brake_ = false;
          // printf("Avoidance...\n");
          // not often happening so wait for now
          // is_avoid_ = false;
          // is_avoiding_ = false;
          // is_brake_ = true;
        }
...
void DynamicPlanning::generateDynamicPath()
{
  ...
  if (is_brake_ == false && is_avoid_ == false)
  {
    ...
    status_ = DynamicPlanning::STATE::RELAYING_I;
  }
  // RELAYING II
  else if (is_brake_ = true && is_avoid_ == false)
  {
    ...
    status_ = DynamicPlanning::STATE::RELAYING_II;
    if (!is_avoiding_)
    {
      printf("Avoiding: opt path wo obs\n");
      publishReferencePath(candidates_[opt_path_wo_obs_].points);
      visualizePathCandidate(opt_path_wo_obs_);
    }
    ...
    {
      printf("Avoiding: opt path w/ obs\n");
  ...
  else if (is_brake_ == false && is_avoid_ == true)
  {
    ...
    if (!is_blocked_)
    {
      ...
      setTargetSpeed(15.0);
      visualizePathCandidate(opt_path_);
      status_ = DynamicPlanning::STATE::RELAYING_III;
    }
    else
    {
      printf("****************BLOCKED****************\n");
      ...
      printf("Following previous optimal path: %d\n", prev_opt_path_);
      status_ = DynamicPlanning::STATE::BLOCKED;
      hardBrake();
    }
  }
...
void DynamicPlanning::sendEstopCommand()
{
  std_msgs::UInt8 msg;
  msg.data = 1;
...
void DynamicPlanning::executeEStop(obj_msgs::Obj obj)
{
  // Estop not enabled
  if (estop_status_ == (uint8_t)0)
  {
    // bool is_veh = isVehicle(obj);
    double arc_diff = obj.s - ego_.s;
    // Goal
    if (obj.type == OType::GOAL)
    {
      if (ego_speed_ > 0 && ego_speed_ <= 10.0 && arc_diff >= 0 && arc_diff <= 11 && ego_.lat_offset <= HALF_LANE)
      {
        printf("Too close to the goal --> ESTOP\n");
        std::vector<geometry_msgs::Point> ref_path;
        for(int i = ego_.index; i < global_trajectory_.waypoints.size(); ++i)
        {
          if (i > obj.index)
    ...
    // else if (is_veh)
    // {
    //   if (ego_speed_ > 0 && ego_speed_ <= 10.0 && arc_diff >= 0 && arc_diff <= 11)
    //   {
    //     printf("Too close to the vehicle --> ESTOP\n");
    //     std::vector<geometry_msgs::Point> ref_path;
    //     for(int i = ego_.index; i < global_trajectory_.waypoints.size(); ++i)
    //     {
    //       if (i > obj.index)
  ...
  // Estop currently enabled
  else
  {
    double curr_time = ros::Time::now().toSec();
    if (estop_timer_ == 0.0)
    {
      estop_timer_ += ros::Time::now().toSec() - curr_time;
    }
    else
    {
      estop_timer_ += curr_time - prev_estop_time_;
    }
    prev_estop_time_ = curr_time;
    if (estop_timer_ > 1.5)
    {
      estop_timer_ = 0.0;
      printf("Disabling Estop...\n");
      sendEstopCommand();
    }
  }
  printf("estop_timer: %f\n", estop_timer_);
}

void DynamicPlanning::sendReachedGoal()
{
  if (ego_speed_ == 0.0)
  {
    std_msgs::UInt8 msg;
...
void DynamicPlanning::executeStopAndGo(obj_msgs::Obj obj, double d_tilda, double arc_diff)
{
  // Goal인데 못들어가고 있는 경우
  if (obj.type == OType::GOAL && d_tilda >= 0)
  {
    double curr_time = ros::Time::now().toSec();
    if (ego_speed_ == 0.0)
    {
      if (stop_go_timer_ == 0.0)
      {
        stop_go_timer_ += ros::Time::now().toSec() - curr_time;
      }
      else
      {
        stop_go_timer_ += curr_time - prev_acc_time_;
      }
    }
    else if (ego_speed_ != 0.0 && !is_stop_go_)
    {
      stop_go_timer_ = 0.0;
    }

    printf("STOP GO waiting time: %f\n", stop_go_timer_);
    printf("Distance to goal: %f m \n", arc_diff);
    // ROS_WARN("STOP GO waiting time: %f", stop_go_timer_);
    prev_acc_time_ = curr_time;

    // Within ACC region but stopped far from the goal
    if (stop_go_timer_ >= 3.0  && arc_diff >= 17.0) //17.0
    {
      is_stop_go_ = true;
      std::vector<geometry_msgs::Point> ref_path;
      for(int i = ego_.index; i < global_trajectory_.waypoints.size(); ++i)
      ...
      publishReferencePath(ref_path);
      minAccel();
      // ROS_WARN("Execute Stop and Go");
    }
    // Close to the goal
    else if (stop_go_timer_ >= 3.0 && arc_diff < 17.0) // 17.0
    {
      is_stop_go_ = true;
      std::vector<geometry_msgs::Point> ref_path;
      for(int i = ego_.index; i < global_trajectory_.waypoints.size(); ++i)
      ...
      // Goal link와 가깝지 않으면 더 앞으로 가기 (TEST)
      double distance_to_goal_link = getDistanceToLink(ego_, goal_link_);
      printf("Distance to goal link: %f m\n", distance_to_goal_link);
      if (distance_to_goal_link >= 0.5)
      {
        minAccel();
        printf("Nearby the goal, but far from goal link\n");
      }
      //
      // // Bezier 위에 있을 시 앞으로 더 가기 (TEST)
      // if (global_trajectory_.waypoints[ego_.index].type == 1)
      // {
      //   minAccel();
      //   printf("Nearby the goal, but BEZIER\n");
      // }
      else
      {
        sendReachedGoal();
        printf("Nearby the goal: STOP\n");
        hardBrake();
      }
      // ROS_WARN("Nearby the goal");
    }
    else
    {
      is_stop_go_ = false;
...
void DynamicPlanning::executeAdaptiveCruiseControl(obj_msgs::Obj obj)
{
  ...
  // double arc_diff = std::get<0>(locateObjectFromEgo(obj)); // 뮬채꺼자 거리
  double arc_diff = std::get<0>(locateObjectBackFromEgo(obj)); // 뮬채꺼자 거리
  ...
  executeStopAndGo(obj, d_tilda, arc_diff);
  if (is_stop_go_)
  {
    status_ = DynamicPlanning::STATE::STOP_GO;
    return;
  ...
  {
    bool is_prev_veh = isVehicle(prev_obj_);
    bool is_curr_veh = isVehicle(obj);
    printf("Prev: %d, Curr: %d\n", prev_obj_.id, obj.id);
    if (is_prev_veh != is_curr_veh)
    {
      printf("Tracking object changed\n");
      dr_ = arc_diff;
      if (dr_ <= 25.0)
      {
        // Publish acceleration data
  ...
  // dr_dot_ = c / 2.0 * d_tilda * d_tilda - 9.72222;
  ...
  // At the inclined surface (Temporal)
  else if (u < 0 && u > -0.2 && ego_speed_ == 0.0)
  {
    u = -0.4;
  }
...
double DynamicPlanning::getBezierSpeed()
{
  ...
  if (is_bezier)
  {
    ...
    double speed = global_trajectory_.waypoints[start_point].speed / 1.5;
...
void DynamicPlanning::searchObjAhead()
{
  ...
  for(int i = 0; i < obj_list.size(); i++)
  {
    ...
    std::tuple<double, double> obj_info = locateObjectBackFromEgo(obj);
...
void DynamicPlanning::printObjStatus()
{
  ...
  {
    obj_msgs::Obj obj = veh_list_[i];
    printf("Veh %d, type: %d, index %d, arc %f, lat offset %f m, is_wait: %d\n", obj.id, obj.type, obj.index, obj.s, obj.lat_offset, obj.is_wait);
  }
  printf("================================================================\n");
  ...
  for(int i = 0; i < traffic_list_.size(); i++)
  {
    ...
    printf("Traffic light: id: %d, index %d, arc %f, lat offset %f m, is_wait: %d\n", traffic_light.id, traffic_light.index, traffic_light.s, traffic_light.lat_offset, traffic_light.is_wait);
  }
  printf("================================================================\n");
  printf("==============================Stop Node===========================\n");
  for(int i = 0; i < stop_nodes_.size(); i++)
  {
    ...
    printf("Stop node id: %d: index %d, arc %f, lat offset %f m, is_wait: %d\n", node.id, node.index, node.s, node.is_wait);
  }
  printf("================================================================\n");
}
...
bool DynamicPlanning::isBezierAhead(int check_index)
{
  for(int i = ego_.index; i < ego_.index + check_index; i++)
  {
    if (i < global_trajectory_.waypoints.size())
    {
      if (global_trajectory_.waypoints[i].type == 1)
      {
        return true;
      }
    }
...
bool DynamicPlanning::collisionWithApproachingObject(double checking_range)
{
  ...
  printf("CWA: ref index: %d\n", ref_index);
  // Check appraoching objects
  for(int i = 0; i < veh_list_.size(); i++)
  {
    obj_msgs::Obj obj = veh_list_[i];
    double arc_diff = std::get<0>(locateObjectBackFromEgo(obj));
    if ((obj.type == OType::FRONTOBJ && abs(arc_diff) <= 20.0) || obj.type == OType::APPROACHING)
    {
      printf("CWA: type: %d, diff: %f, lat: %f\n", obj.type, arc_diff, obj.lat_offset);
      for (int j = ego_.index; j <= ref_index; j++)
      {
        ...
        for (double t=0.0; t <= 10.0; t+= 0.1)
        {
          ...
          {
            printf("<<<GLOBAL>>>\n");
            printf("Approaching: collision..., arc_diff: %f, type: %d, id: %d\n", arc_diff, obj.type, obj.id);
            printf("Collision point -> x: %f, y: %f, ttc: %f\n", x, y, t);
            return true;
          }
        }
      }
      if (arc_diff < 0 && abs(obj.lat_offset)> HALF_LANE)
      {
        printf("<<<GLOBAL>>>\n");
        printf("Approaching: back collision, arc_diff: %f, type: %d, lat_offset: %f\n", arc_diff, obj.type, obj.lat_offset);
        return true;
      }
    }
  }
...
void DynamicPlanning::switchState()
{
  if (obj_ahead_)
  {
    is_wait_status_ = headway_obj_.is_wait;
    double ref_speed_ms = ref_speed_ * 1000.0 / 3600.0;
    double arc_diff = std::get<0>(locateObjectBackFromEgo(headway_obj_));
    double d_nominal = getNominalDistance(ref_speed_ms);
    double d_tilda = d_nominal - arc_diff;
    ...
    else
    {
      ...
      else
      {
        ...
        if (is_on_path)
        {
          ...
          double looking_range;

          if (!is_bezier_ahead_)
          {
            printf("switchState: bezier does not exist\n");
            if (arc_diff >= 50.0)
            {
              looking_range = 50.0;
            }
            ...
            {
              looking_range = arc_diff;
            }
          }
          else
          {
            printf("switchState: bezier ahead\n");
            looking_range = arc_diff;
          }

          printf("Looking range: %f m\n", looking_range);
          bool is_collision_front = collisionWithApproachingObject(looking_range);
          // bool is_collision_bezier;
          if (is_collision_front)
          {
            printf("Collision in the global path\n");
            // ROS_WARN("Collision in the global path");
            state_ = DynamicPlanning::STATE::HARD_BRAKE;
            status_ = DynamicPlanning::STATE::HARD_BRAKE;
          }
          else
          {
...
bool DynamicPlanning::isDeadEnd()
{
  ...
  for(int i=0; i < raw_veh_list_.size(); i++)
  {
    ...
    if (obj.s >= ego_.s)
    {
      printf("Obj at: %f, deadend_obj at: %f\n", obj.s, obj_deadend_.s);
      obj_msgs::Obj bl;
      obj_msgs::Obj br;
      obj_msgs::Obj ul;
      obj_msgs::Obj ur;
      ...
      if (abs(obj.lat_offset) <= 1.5) // 1.5
      {
        ...
        if (lane_status == LControl::BOTH)
        {
          ...
          if ((bl_lat > deadend_thres_ || ul_lat > deadend_thres_) && (ur_lat < -deadend_thres_ || br_lat < -deadend_thres_))
          {
            // ROS_WARN("Deadend: all three links blocked");
            printf("Deadend: all three links blocked\n");
        ...
        else if (lane_status == LControl::RIGHT)
        {
          ...
          if(br_lat < -deadend_thres_ || ur_lat < -deadend_thres_)
          {
            // ROS_WARN("Deadend: right and current links are blocked");
            printf("Deadend: right and current links are blocked\n");
            if (obj.s <= obj_deadend_.s)
            {
              obj_deadend_ = obj;
            }
        ...
        else if (lane_status == LControl::LEFT)
        {
          ...
          if(bl_lat > deadend_thres_ || ul_lat > deadend_thres_)
          {
            // ROS_WARN("Deadend: left and current links are blocked");
            printf("Deadend: left and current links are blocked\n");
            if (obj.s <= obj_deadend_.s)
            {
              obj_deadend_ = obj;
            }
        ...
        else if (lane_status == LControl::KEEP)
        {
          ...
          printf("Deadend: current link is blocked\n");
          if (obj.s <= obj_deadend_.s)
          {
            obj_deadend_ = obj;
          }
      ...
      else
      {
        ...
        if (abs(bl_lat) < deadend_thres_ || abs(ul_lat) < deadend_thres_ || abs(br_lat) < deadend_thres_ || abs(ur_lat) < deadend_thres_)
        {
          // ROS_WARN("Deadend: blocking the current link");
          printf("Deadend: blocking the current link\n");
  ...
  printf("Deadend: obj id: %d\n", obj_deadend_.id);
  if (obj_deadend_.id == 0)
  {
    // ROS_WARN("Deadend: Free to go");
    printf("Deadend: Free to go\n");
    return false;
  }
  else
  {
...
void DynamicPlanning::requestNewPlan(obj_msgs::Obj obj)
{
  int left_point_size = (global_trajectory_.waypoints.size() - ego_.index -1) * 2;
  geometry_msgs::Pose curr_pose = ego_.pose;
  geometry_msgs::Pose obj_pose = obj.pose;
  path_msgs::Avoidance avoidance_msg;
  avoidance_msg.vehicle_pose = curr_pose;
  avoidance_msg.obj_pose = obj_pose;
  avoidance_msg.left_traj = left_point_size;
  replan_pub_.publish(avoidance_msg);
}
...
bool DynamicPlanning::isAdaptiveCruiseControlFinish()
{
  if (state_ == DynamicPlanning::STATE::CRUISING)
  {
    ...
    if (is_veh)
    {
      printf("Wait status: headway: %d, is_wait: %d\n", headway_obj_.id, headway_obj_.is_wait);
    ...
    if (acc_timer_ >= avoidance_wait_time_thres_ && !headway_obj_.is_wait)
    {
      printf("Requesting New Plan\n");
      requestNewPlan(headway_obj_);
    }
    else
    {
      // look_further_ = false;
...
double DynamicPlanning::getDistanceToLink(obj_msgs::Obj obj, path_msgs::Link link)
{
  if (link.id != "")
  {
    double min_dist = getDistance(obj.pose.position.x, obj.pose.position.y, link.geometry[0].x - offsetX_, link.geometry[0].y - offsetY_);
    for(int i = 1; i < link.geometry.size(); i++)
    {
      double dist = getDistance(obj.pose.position.x, obj.pose.position.y, link.geometry[i].x - offsetX_, link.geometry[i].y - offsetY_);
      if (dist < min_dist)
      {
        min_dist = dist;
      }
