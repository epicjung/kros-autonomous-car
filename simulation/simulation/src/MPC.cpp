...
vector<double> MPC::Solve(Eigen::VectorXd x0, Eigen::VectorXd coeffs, double target_speed, double target_acc, double current_acc) {
  ...
  for (i = v_start; i < cte_start; i++)
  {
    ...
    if (target_acc != 0)
    {
      vars_upperbound[i] = target_speed;
    }
    else
    {
      vars_upperbound[i] = vars[v_start] + (target_speed - vars[v_start]) / (N-1) * (i-v_start);
    }

    // vars_upperbound[i] = 50.0*1000.0/3600 - (50.0*1000.0/3600-target_speed) / (N-1) * (i-v_start);
  }
  ...
  for (i = delta_start; i < a_start; i++) {
    vars_lowerbound[i] = -0.436381944;
    vars_upperbound[i] = 0.436381944;
  }
  ...
  for (i = a_start; i < n_vars; i++) {
    ...
    // printf("target_acc: %f, current_acc: %f\n", target_acc, current_acc);
    if (target_acc < 0.0)
    {
      vars_upperbound[i] = target_acc;
      // vars_upperbound[i] = current_acc + 1.0 * (target_acc - current_acc) / (N-1) * (i-a_start);
      // printf("Upperbound: %f\n", vars_upperbound[i]);
    }
    // else if (target_acc > 0.0)
    // {
    //   vars_lowerbound[i] = target_acc;
    // }
  }
